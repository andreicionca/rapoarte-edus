...
function displayMediaGenerala() {
  const mediaGeneralaDiv = document.getElementById('student-msg-media-generala');
  ...
  studentMsgAverage.style.width = `${mediaHeader.getBoundingClientRect().width}px`;

  const studentName = document.getElementsByClassName('student-upper')[0].textContent;

  // Check if the student name exists in studentsStatistics
  if (studentsStatistics.hasOwnProperty(studentName)) {
    const generalAverage = parseFloat(studentsStatistics[studentName].generalAverage);
    studentMsgAverage.textContent = generalAverage.toFixed(2);
    // after student name i want to display the position in class
    const studentNameElement = document.getElementsByClassName('student-upper')[0];
...
${Object.keys(gradesDataToRender[firstStudentGrades]['Disciplines'])
  .map((discipline) => {
    const average = gradesDataToRender[firstStudentGrades]['Disciplines'][discipline].average;
    const disciplineName =
      average > 4.99 || average === '' ? discipline : `${discipline} (corigent)`;
    ...
    for (let i = 0; i < numberofGradesCells; i++) {
      tableGradesRows += `<td class="col-centru">${
        gradesDataToRender[firstStudentGrades]['Disciplines'][discipline].grades[i] || ''
      }</td>`;
    }

    tableGradesRows += `<td class="col-centru bold-text media-cell" style="background-color:#FFCDD2
    ;">${gradesData[firstStudentGrades]['Disciplines'][discipline].average || ''}</td>`;
...
`;
  ...
  const student = studentTotalsOriginal.find((s) => s.name === firstStudentGrades);
  if (student) {
...
function updateTable() {
  ...
  if (selectedClasa !== 'all') {
    filteredData = filteredData.filter((row) => row[4].trim() === selectedClasa.trim());
  }
...
sortoptionsSelect.addEventListener('change', () => {
  ...
  if (sortOption === 'motivate') {
    ...
    let totalTds = document.getElementById('data-table-totals').querySelectorAll('td:nth-child(5)');
...
document.getElementById('next-table-button').addEventListener('click', scrollToNextTable);

function scrollToNextTable() {
  if (!nextTableReached) {
    document.getElementById('elev-select').scrollIntoView({ behavior: 'smooth' });
    nextTableReached = true;
    document.getElementById(
      'next-table-button'
    ).innerHTML = ` <ion-icon name="arrow-up-outline"></ion-icon>`;
  } else {
    document.getElementById('show-tables').scrollIntoView({ behavior: 'smooth' });
